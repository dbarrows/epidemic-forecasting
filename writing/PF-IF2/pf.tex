...
\section{Fitting an SIR Model to Synthetic Epidemic Data with IF2}
    ...
    Bootstrapping from the final particle sample gives us awesome stuff too as we can see in Figure \ref{if2plot}. 

    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{./images/if2plot.pdf}
        \caption{IF2 estimate of the system state at each time point using bootstrapping. Ribbon shows 2.5\% and 97.5\% quantiles.}
        \label{if2plot}
    \end{figure}
...
\begin{appendices}
    ...
    \lstinputlisting[style=Rsty]{../../code/if2/rcpptest/bootstrap.r}

    \section{Full C++ code}

    Stan model code to be used with the preceding R code.

    \lstinputlisting[style=Cppsty]{../../code/if2/if2.cpp}
