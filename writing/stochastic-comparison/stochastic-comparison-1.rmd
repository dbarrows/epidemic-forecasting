...
We can observe the effect of $\eta$ visually by setting $\eta$ to each extreme value and then to an intermediate value and plotting the results. The anchoring value $\beta_0$ was set to $\beta_0 = R_0 \cdot r / N$ where $R_0 = 3.0$, $r = 0.1$ and $N = 500$, giving $B_0 = 6 \times 10^{-4}$. The noise parameter $\rho^2$ was set to $0.5$.
...
Note that when $\eta = 1$ we have $\beta_t = \exp( \log(\bar{\beta}) + \epsilon_t )$, constituting a log-normal distribution with location $\mu = log(\bar{\beta})$ and scale $\sigma = \rho$. The resulting distribution has a theoretical mean of $\exp(\mu + \sigma^2/2)$ which evaluates to
...
and a theoretical standard deviation of $\sqrt{ (exp(\sigma^2) - 1) \cdot exp(2\mu + \sigma^2)}$ which evaluates to
...
And we can see these values are very close to the experimentally determined ones.
