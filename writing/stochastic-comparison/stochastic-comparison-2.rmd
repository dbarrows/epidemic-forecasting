...
knitr::opts_chunk$set(cache = TRUE, autodep = FALSE, echo = FALSE, warning = FALSE, message = FALSE, error = FALSE)
```
...
```{r, paraboot_fit}
nTraj 		<- 10 	# trajectories for both bootstrapping runs
...
```{r, plot_paraboot}
meanTraj 	<- colMeans(parabootdata)
quantTraj 	<- t(apply(parabootdata, 2, quantile, probs = c(0.025,0.975)))
...
We can fit the model to successively smaller (or larger) time series to see the effect of truncation on forecast SSE. This was done below, with several *(10 right now, I'll run this for a larger number of trials in the future)* new trajectories drawn for each of the desired lengths.
...
Here the solid line shows the IF2 truncation normalized SSEs and the dashed line shows the HMCMC normalized SSEs