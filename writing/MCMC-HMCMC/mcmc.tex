...
\section{Algorithm}
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
            ...
            \Require Initialize $\theta^{(1)}$ by selecting a starting point

            \For{$i$ in $2:N$}
                \State Sample proposed value $\theta^* \sim q(\cdot|\theta^{(i-1)})$
                \State Sample $u \sim \mathcal{U}(0,1)$
                \State Evaluate acceptance ratio $r = \frac{\mathcal{L}(\theta^*)p(\theta^*)}{\mathcal{L}(\theta)p(\theta)}$
                \If{ $u < \min\left\{ 1 , r \right\}$ }
                    \State $\theta^{(i)} = \theta^*$
                \Else{}
                    \State $\theta^{(i)} = \theta^{(i-1)}$
                \EndIf
            \EndFor\label{euclidendwhile}

            \Ensure Chain of samples $(\theta^{(1)},\theta^{(2)},...,\theta^{(N)})$
...
\section{Fitting an SIR Model to Synthetic Epidemic Data}
    ...
    \centering
        ...
        \begin{subfigure}[tr]{0.49\textwidth}
            ...
            %\label{fig:tiger}
        \end{subfigure}
        \begin{subfigure}[bl]{0.49\textwidth}
            \includegraphics[width=\textwidth]{./images/kernelsigma.pdf}
            %\label{fig:mouse}
        \end{subfigure}
        \begin{subfigure}[br]{0.49\textwidth}
            \includegraphics[width=\textwidth]{./images/kernelinfec.pdf}
            %\label{fig:mouse2}
        \end{subfigure}
        \caption{Kernel density estimates produced by Stan}
        \label{kernels}
    \end{figure}
