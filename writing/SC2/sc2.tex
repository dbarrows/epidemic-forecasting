...
\subsection{IF2 Forecasts}
    ...
    We can define a metric to gauge forecast effectiveness by calculating the SSE and dividing that value by the number of values predicted to get the average squared error per point. For the data in Figure [\ref{if2forecast}] the value was $\overline{SSE} = 1.67$.
...
\section{HMCMC}
    ...
    As before we can evaluate the averaged SSE of the forecast for the data shown, giving $\overline{SSE} = 20.27$.
...
\section{Truncation vs. Error}
    ...
    IF2 and HMCMC perform very closely, with IF2 maintaining a small advantage up to a truncation of about 25 data points.
