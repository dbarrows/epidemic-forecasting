...
propagateSIR<-function(tm_strt, tm_end, tm_step, S0, I0, N, D, L, beta, realdata=TRUE){
  # function to integrate to the next time step, for single simulation
  ...
  #         beta: transmission matrix
  # output: S, I, cumulative incidence, for all time steps
  ...
  if (discrete){
    ...
    for (t in tm_vec){
      ...
      ik2=smci-smcr
      ik2a=smci;
      Ts2=Ts1+round(sk2/2,0)
      Ti2=Ti1+round(ik2/2,0)
...
propagateParSIR<-function(tm_strt, tm_end, tm_step, S0, I0, N, D, L, beta, realdata=FALSE){
  ...
  # use SIR model, integrate dicretely with Poisson distributions
  # input: tm_strt: starting time; tm_end: ending time; tm_step: time step
  #         S0, I0: initial states;
  #         S0, and I0 should be passed in as a vector (Np particles)
  #         N: population size
  #         D: infection period, day; L: immune period, day; 
  #         beta: transmission matrix
  # output: S, I, and cumulative incidence, for all time steps
    
  cnt=1;
  # beta stores only data during the time used for the simulation
  ...
  Np=length(S0); # number of particles (i.e., simulations)
