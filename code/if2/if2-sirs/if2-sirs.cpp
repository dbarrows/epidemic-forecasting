...
void exp_euler_SIRS(double h, double t0, double tn, int N, Particle * particle);
...
// [[Rcpp::export]]
Rcpp::List if2_sirs(NumericVector data, int T, int N, int NP, int nPasses, double coolrate) {

	int npar = 9;

	NumericMatrix paramdata(NP, npar);
	NumericMatrix means(nPasses, npar);
	NumericMatrix sds(nPasses, npar);
	...
	for (int n = 0; n < NP; n++) {
		...
		double R0can, rcan, recan, sigmacan, Iinitcan, etacan, berrcan;
		...
		do {
			recan = retrue + retrue*randn();
		} while (recan < 0);
		particles[n].re = recan;
	...
	for (int pass = 0; pass < nPasses; pass++) {
		...
		for (int t = 1; t < T; t++) {
			...
			for (int n = 0; n < NP; n++) {
				...
				exp_euler_SIRS(1.0/7.0, (double) t-1, (double) t, N, &particles[n]);
		...
		means(pass, 2) = pInfo.remean;
		means(pass, 3) = pInfo.sigmamean;
		means(pass, 4) = pInfo.etamean;
		means(pass, 5) = pInfo.berrmean;
		means(pass, 6) = pInfo.Sinitmean;
		means(pass, 7) = pInfo.Iinitmean;
		means(pass, 8) = pInfo.Rinitmean;

		sds(pass, 0) = pInfo.R0sd;
		sds(pass, 1) = pInfo.rsd;
		sds(pass, 2) = pInfo.resd;
		sds(pass, 3) = pInfo.sigmasd;
		sds(pass, 4) = pInfo.etasd;
		sds(pass, 5) = pInfo.berrsd;
		sds(pass, 6) = pInfo.Sinitsd;
		sds(pass, 7) = pInfo.Iinitsd;
		sds(pass, 8) = pInfo.Rinitsd;
	...
	for (int n = 0; n < NP; n++) {
		...
		paramdata(n, 2) = particles[n].re;
		paramdata(n, 3) = particles[n].sigma;
		paramdata(n, 4) = particles[n].eta;
		paramdata(n, 5) = particles[n].berr;
		paramdata(n, 6) = particles[n].Sinit;
		paramdata(n, 7) = particles[n].Iinit;
		paramdata(n, 8) = particles[n].Rinit;
...
void exp_euler_SIRS(double h, double t0, double tn, int N, Particle * particle) {
	...
	for(int i = 0; i < num_steps; i++) {
		...
		//double Bfac = 0.5 - 0.95*cos( (2.0*M_PI/365)*(t0*num_steps+i) )/2.0;
...
void perturbParticles(Particle * particles, int N, int NP, int passnum, double coolrate) {
    ...
    double spreadre 	= coolcoef * retrue / 10.0;
    ...
    for (int n = 0; n < NP; n++) {
    	...
    	do {
    		recan = particles[n].re + spreadre*randn();
    	} while (recan < 0);
    	particles[n].re = recan;
...
void particleDiagnostics(ParticleInfo * partInfo, Particle * particles, int NP) {
    ...
    for (int n = 0; n < NP; n++) {
    	...
    	resd 	+= ( particles[n].re - rmean ) * ( particles[n].re - rmean );
