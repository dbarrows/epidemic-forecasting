...
#define NP 		3*1024		// number of particles per cell
...
void exp_euler_SIR(float h, float t0, float tn, Particle * particles, int pID, float * I_last, float * Beta_last, int adjMat[][8], int * nNeibVec, int cID);
...
int main(int argc, char *argv[]) {
	...
	cout << "About to allocate big things" << endl;
	...
	cout << "Done allocating big things" << endl;

	cout << "About to populate adjacency matrix" << endl;
	...
	cout << "Done populating adjacency matrix" << endl;
	...
	*/
	...
	for (int t = 0; t < T; t++) {
		y_true[t] 	= truevals[t*dim*dim+cID];
		y_noise[t] 	= datavals[t*dim*dim+cID];
	}
	*/
	...
	cout << "About to initialize particles" << endl;
	...
	cout << "About to generate initial estimates" << endl;
	...
	cout << "Done generating initial estimates" << endl;

	cout << "About to try filtering over time [0," << T << "]"<< endl;
	...
	for (int t = 1; t < T; t++) {
		...
		for (int cell = 0; cell < nCells; cell++) {
			...
			for (int n = 0; n < NP; n++) {
				...
				exp_euler_SIR(1.0/10, 0.0, 1.0, &particles[cell][0], n, y_est_last, B_est_last, adjMat, nNeibVec, cell); 	// project particle forward
